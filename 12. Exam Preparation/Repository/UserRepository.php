...
class UserRepository
{
    ...
    public function check(string $username) :UserDTO{
        $stm= $this->db->query('SELECT USER_ID,USERNAME,FIRST_NAME,LAST_NAME,PASSWORD FROM users 
                                      WHERE USERNAME= :username');
        $result=$stm->execute(['username' => $username]);
        return $result->getOne(UserDTO::class);
    }
    public function getOne(int $user_id) {
        $stm= $this->db->query('SELECT USER_ID,USERNAME,FIRST_NAME,LAST_NAME FROM users 
